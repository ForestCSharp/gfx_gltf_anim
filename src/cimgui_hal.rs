...
impl CimguiHal {
	pub fn new( device_state : &gfx_helpers::DeviceState, 
	...
	pub fn render(&mut self, cmd_buffer : &mut hal::command::CommandBuffer<B, hal::General>, framebuffer: &<B as Backend>::Framebuffer, device_state : &gfx_helpers::DeviceState, transfer_queue_group : &mut hal::QueueGroup<B, hal::General>) {
		unsafe {
			igEndFrame();
